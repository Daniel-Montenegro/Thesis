...
spin_disk_bh = Sn_bh.BH_AngMomGasCells ##momentum angular disco acrecion
...
Mass_star_halo = sub.SubhaloMassType[:,4]
...
for k in range(len(r_bh)):
    ...
    Mass_stelar_halo_new.append(sub.SubhaloMassType[id,4]) ##Mass stelar
...
Mass_stelar_halo_new = np.reshape(Mass_stelar_halo_new,(len(r_bh),1))
...
np.savetxt('Data(halo-bh-stelar)_new.txt',
                   np.column_stack((Mass_halo_new, r_halo_new , vel_dis_halo_new, vel_max_halo_new,spin_halo_new,Mass_stelar_halo_new,  Mass_bh, r_bh, vel_disp_bh, Spin_bh, spin_disk_bh )), header= '#',delimiter=" ", comments="# Mass - r_halo(x,y,x) - vel_dis_halo - vel_max - spin_halo(i,j,k) Mass_stelar - Mass_bh - r_bh(x,y,x) - vel_disp_bh -  Spin_bh(i,j,k) - spin_disk_bh(i,j,k)")
