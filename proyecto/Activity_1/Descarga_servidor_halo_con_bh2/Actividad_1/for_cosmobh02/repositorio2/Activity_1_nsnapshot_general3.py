...
Sn_bh = gadget.Snapshot('/home/dmontenegro/Data/Sims512/cosmobh02/snapdir_015/snap_015.0.hdf5', parttype=[5], combineFiles=True, verbose=True)
...
plt.plot(np.log10(vel_disp_bh), (alpha+beta*np.log10(vel_disp_bh/sigma_0)), label='linea teorica')
...
sub = gadget.Subfind('/home/dmontenegro/Data/Sims512/cosmobh02/groups_015/fof_subhalo_tab_015.0.hdf5' ,combineFiles=True)
...
plt.xlabel("$\log_{10}(Mass_{halo})[M_{\odot}]$")
plt.savefig("histo_Mass_subhalos.png")
...
ax.scatter(r_bh[:,0], r_bh[:,1], r_bh[:,2], s=1, c= 'r', alpha=1 , label="bh")
ax.scatter(r_halo_new2[:,0], r_halo_new2[:,1], r_halo_new2[:,2], s=7, c='blue' , alpha= 0.5,label="halo")
ax.legend()
...
# Parametro para las lineas teoricas

M_halo_teorica = 2e9*(0.7**(-2))*(vel_max_halo/100.)**4 #para la V maxima
#referencia: Extragalactic_Astronomy_and_Cosmology-Peter Schneider-pag.107
M_halo_disp_teorica = 1e12*1.4*(vel_dis_halo/200.)**(3.32) ## Para la dispersion
#referencia: https://academic.oup.com/mnras/article/341/4/L44/1039943
...
plt.plot(np.log10(vel_max_halo),np.log10(M_halo_teorica), label='teorico')
...
plt.plot( np.log10(vel_dis_halo), np.log10(M_halo_disp_teorica), label='teorico')
...
#popt, pcov = curve_fit(func_disp_bh, np.log10(vel_disp_bh),np.log10(Mass_bh))
#popt_velmax, pcov_velmax = curve_fit(f_vel_max_halo, np.log10(vel_max_halo_new2),np.log10(Mass_halo_new2))
popt2, pcov2 = curve_fit(f_MbhVsMhalo, Mass_halo_new2, Mass_bh )

plt.figure()
plt.loglog(Mass_halo_new2,Mass_bh,'.', label="Dato")
#plt.plot(xdata, func(xdata, *popt), 'g--',
plt.loglog(Mass_halo_new2, f_MbhVsMhalo(Mass_halo_new2, *popt2), ".k", label='ajuste')
plt.ylabel("$Mass_{bh}$")
...
plt.figure()
plt.plot(Mass_halo_new2,(Mass_bh/Mass_halo_new2),'.')
plt.xscale("log")
plt.yscale("log")
